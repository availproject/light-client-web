    ...
    <div className="flex flex-row  overflow-auto whitespace-nowrap ">{
      blockList.map((block: BlockType, index: number) => (
        <div key={index} className="flex flex-col items-center justify-center">
          <h3 className={`z-50 underline text-md ${blockList.length > index + 1 ? 'text-white' : 'text-[#3CBBF9] animate-pulse'} text-opacity-70`}><a target="_blank" href={props.network === "Mainnet" ? `https://avail.subscan.io/block/${block.number}` : `https://avail-turing.subscan.io/block/${block.number}`}>#{block.number}</a></h3>
